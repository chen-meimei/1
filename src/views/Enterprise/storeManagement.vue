      ...
      <div class="form-area" style="width:100%" >
        <div class="form-row" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
          ...
          <div class="label-2-select">
            <label>状态</label>
            <a-select v-model="status" placeholder="" @change="statusHandleChange" :labelInValue="true">
        ...
        <a-table style="width:100%;height:100%"
          ...
          <template slot="action" slot-scope="text,record">
            <div style="display:flex;justify-content:space-around">
              ...
              <a-button type="danger" style="padding:0 10px;" @click="stopButton(record.key)" :id="record.key" v-if="record.status=='启用'" >关闭</a-button>
              <a-button type="primary" style="padding:0 10px;" @click="startButton(record.key)" :id="record.key" v-if="record.status=='关闭'">启用</a-button>
            </div>
          </template>
...
<!-- 	  查看弹窗 -->
      <a-modal
        ...
        <div class="modal-body">
          <a-form-model :model="form2" :rules="checkRules" ref="form2" style="width:360px" :labelCol="{span:6}" :wrapperCol="{span:18}">
            <a-form-model-item  label="渠道名称">
              <a-input style="width: 240px;" v-model="form2.Channelname"/>
            </a-form-model-item>
            <a-form-model-item  label="门店电话" prop="storeTel" >
...
<!--            <a-form-item  label="门店类型">
              <a-select placeholder="全部"  :labelInValue="true" :default-value="form2.storeType"  @change="changeForm2StoreType"   style="width: 240px;">
           ...
           <a-form-model-item  label="门店类型">
            ...
            <a-form-model-item  label="剩余次数" prop="restTime">
              <a-input style="width: 240px;" v-model="form2.restTime"/>
            </a-form-model-item>
<!--            <a-form-model-item  label="状态" >
                <a-select placeholder="全部" :labelInValue="true"  :default-value="form2.status" @change="changeForm2Status" style="width:240px;">
...
<!-- 	新建门店弹窗 -->
      <a-modal
        ...
        :maskStyle="{'background-color':'rgb(0,0,0,0.2)'}">
        <div class="modal-body">
          <a-form-model style="width:360px" :rules="checkNewRules" :model="checkNew" ref="checkNew" :labelCol="{span:6}" :wrapperCol="{span:18}">
            <a-form-model-item  label="渠道名称">
            ...
            <a-form-model-item  label="联系人电话" prop="newLinkmanTel">
              <a-input style="width: 240px;"  placeholder=" " v-model="checkNew.newLinkmanTel" />
            </a-form-model-item>
            <a-form-model-item  label="联系人邮箱">
              <a-input style="width: 240px;"  placeholder=" " v-model="newLinkmanMail" />
            </a-form-model-item>
            <a-form-model-item  label="结算模式">
            ...
            <a-form-model-item  label="单次训练价格" prop="newSingletrainPrice">
              <a-input style="width: 240px;"  placeholder=" " v-model="checkNew.newSingletrainPrice"  />
            </a-form-model-item>
            ...
            <a-form-model-item  label="开户银行账号">
              <a-input style="width: 240px;"  placeholder=" " v-model="newBankNumber" />
            </a-form-model-item>
            <a-form-model-item  label="企业开户名称">
...
<!-- 	注册视光师弹窗 -->
      <a-modal  
        ...
        :maskStyle="{'background-color':'rgb(0,0,0,0.2)'}">
        <div class="modal-body">
          <a-form-model style="width:360px" :model="checkNewVision" :rules="checkVisionRules" ref="form2" :labelCol="{span:6}" :wrapperCol="{span:18}">
            <a-form-model-item  label="手机号" prop="newVisionTel">
              <a-input style="width: 240px;"  placeholder=" " v-model="checkNewVision.newVisionTel"/>
            </a-form-model-item>
           ...
           <a-table 
            ...
            <span slot="action" >
              <a-popconfirm
                    ...
                    okText="确定"
                    cancelText="取消"
                    @confirm="() => visionDelete(record.key)"
                  >
                  <!-- !!!此处警告，不要再改动样式!!! -->
                <div style="color:#0F79F9;cursor:pointer;">删除</div>
              </a-popconfirm>
            </span>
            <!-- 用表格的footer来添加视光师 -->
...
const data_vision = [
  ...
  export default {
    data() {
      let check = (rule, value, callback) => {   //定义校验restTime的验证规则
        value = Number(value);
		console.log(value)
        if (isNaN(value)) {
    ...
    methods:{	
      statusHandleChange(value) {
        this.statusLabel=value.label.trim();
        this.status=value;
      },
      ...
      search:function(){
       ...
       let search_data = data;
        ...
        let targetLabel=this.statusLabel;
        if( target1 !== ''){
...
/*      delete_vision(index) {
     ...
     // },
    visionDelete(key) {
      this.data2 = data2.filter(item => item.key !== key);
      //this.data2.splice(key, 1,0);
    },
      //点击重置
      resetInfo(){
...
//视光师弹窗重置
    ...
    //记录form2状态的改变
      changeForm2Status(value){  
        //console.log(value);
        this.form2.status = value;
        //console.log(this.form2.status);
      },
    //记录form2门店类型的改变
      changeForm2StoreType(value){  
        //console.log(value);
        this.form2.storeType = value;
        //console.log(this.form2.status);
      },
      //查看
      ...
      newStore() {
		...
		this.newAddress='';
		this.newLinkmanName='';
		this.checkNew.newLinkmanTel=null;
		this.newLinkmanMail='';
		this.newPattern=undefined;
		this.checkNew.newSingletrainPrice=null;
		this.newProportion='';
		this.newStorename='';
      ...
      regVisionstaff() {
        this.visible3 = true;
        this.checkNewVision.newVisionTel = null;
        this.newVisionName ='';
        this.newVisionName ='';
        this.newVisionMail = '';
      },
...
/*      visionStaffDelete:function(){
	...
	//新建门店提交
      handleOk2() {
        this.$refs.checkNew.validate(valid => {
          if (valid) {
              let item={
				...
				linkmanTel:this.checkNew.newLinkmanTel,
	...
	//注册视光师提交
      handleOk3() {
        ...
        this.data_vision[this.data_vision.length-1].number =this.data_vision.length;
...
//停用启用按钮
    ...
    //导出功能先关闭
      ...
      //     temp = [temp.Channelname, temp.storeTel, temp.storeName, temp.storeType, temp.storeLinkman,temp.address,temp.restTime,temp.status,temp.visionStaff,temp.headstoreID,temp.createTime];
