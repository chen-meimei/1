...
<!--      请在此处填充内容-->
      <div class="form-area" style="width:100%" >
        <div class="form-row" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
          ...
          <div class="label-3-input" help="请输入数字！">
            <label>渠道手机号</label>
            <a-input v-model="form.channelTel"/>
          </div>
          <div class="label-4-select">
            <label>结算模式</label>
            <a-select v-model="pattern" placeholder="" @change="patternHandleChange" :labelInValue="true">
...
<!-- 查看弹窗 -->
      <a-modal
        ...
        <div class="modal-body">
          <a-form-model :model="form2" :rules="checkRules" style="width:360px" :labelCol="{span:7}" :wrapperCol="{span:17}">
            <a-form-model-item  label="渠道名称">
              <a-input style="width: 240px;" v-model="form2.channelName"/>
            </a-form-model-item>
            <a-form-model-item  label="渠道手机号" prop="channelTel">
...
<!--            <a-form-model-item  label="模式">
            ...
            <a-form-model-item  label="模式" >
                <a-select placeholder="" :labelInValue="true"  :default-value="form2.pattern" @change="changeForm2Pattern" style="width:240px;">
                  <a-select-option value="buyOut">
                    买断
                  </a-select-option>
                  <a-select-option value="secondCard">
                    次卡
                  </a-select-option>
              </a-select>
            </a-form-model-item>
            <a-form-model-item  label="单次训练价格" prop="singletrainPrice">
...
<!-- 新建渠道弹窗 -->
      <a-modal
        ...
        :maskStyle="{'background-color':'rgb(0,0,0,0.2)'}">
        <div class="modal-body">
          <a-form-model style="width:360px" :rules="checkNewRules" :model="checkNew" :labelCol="{span:7}" :wrapperCol="{span:17}">
            <a-form-model-item label="渠道名称">
            ...
            <a-form-model-item  label="结算模式">
              <a-select  placeholder="全部"  @change="handleChangeNewPattern"  :labelInValue="true"  v-model="newPattern" style="width: 240px;">
                <a-select-option value="buyOut">买断</a-select-option>
                <a-select-option value="secondCard">次卡</a-select-option>
              </a-select>
            </a-form-model-item>
...
/*     scopedSlots:{
  ...
  export default {
    data() {
      let theCheck = (rule, value, callback) => {   //定义校验的验证规则
        value = Number(value);
        console.log(value);
        if (isNaN(value)) {
      ...
      return {
        ...
        checkRules: { //定义总的校验规则
          restTime: [
            {validator:theCheck, trigger:'blur'},
          ],
          channelTel: [
            {validator:theCheck, trigger:'blur'},
          ],
...
/*      handleChangeStatus(value) {
      ...
      changeForm2Pattern(value){  //记录form2结算模式的改变
        console.log(value);

        this.Labelform2Pattern=value.label.trim();
        //this.form2.pattern = value;
...
//console.log(this.form2.pattern);
        ...
        this.$refs.form2.validate(valid => {
          if (valid) {
              let item1={
                  ...
                  pattern:this.Labelform2Pattern,
        ...
        this.$refs.checkNew.validate(valid => {
          if (valid) {
               let item={
                 ...
                 pattern:this.Label1,
                 singletrainPrice:this.checkNew.newSingletrainPrice,
