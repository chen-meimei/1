...
<script>
  ...
  export default {
    ...
    created() {
      axios({
        method:'post',
        url:'http://java.xixibackup.me:8080/accessories/findAll',
        data:{
          page:1,
          page_size:10,
        }
      ...
      //this.data = data.data.info; //我这儿因为需要改动了下，自己改回来
    },
    methods:{
      //点击搜索按钮
      search:function(){
      ...
      upButton(id){
        ...
        axios({
          method:'post',
          url:'http://java.xixibackup.me:8080/accessories/modify',
          data:{
            name:check[0].name,
            cash_pledge:check[0].cash_pledge,
            prime_cost:check[0].prime_cost,
            real_price:check[0].real_price,
            status:0,
            accessorie_id:check[0].id,
          }
        }).then(res=>{
          console.log(res)
        }).catch(function (error) { // 请求失败处理
        ...
        this.$refs.checkForm.validate(valid=>{
          if(valid){
            ...
            axios({
              ...
              data:{
                name:this.parts,
                //cash_pledge:this.checkForm.deposit.toFixed(2),
                cash_pledge:this.checkForm.deposit,
                prime_cost:this.checkForm.inPrice,
                real_price:this.checkForm.realPrice,
                status:this.data[this.theKey-1].status,
                accessorie_id:this.theKey,
              }
            }).then(res=>{
              console.log(res)
            }).catch(function (error) { // 请求失败处理
      ...
      createOk() {
        this.$refs.createCheckForm.validate(valid=>{
          if(valid){
            let item={
              ...
              name:this.newParts,
              cash_pledge:this.createCheckForm.newDeposit,
              prime_cost:this.createCheckForm.newInPrice,
              real_price:this.createCheckForm.newRealPrice,
              status:'出售中',
              ctime:formatTime(time),
            };
        this.data.push(item);
        ...
        axios({
              method:'post',
              url:'http://java.xixibackup.me:8080/accessories/create',
              data:{
                name:this.newParts,
                cash_pledge:this.createCheckForm.newDeposit,
                prime_cost:this.createCheckForm.newInPrice,
