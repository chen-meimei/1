...
<!--表格显示-->
      <div class="table-area" style="width:100%;margin-top:20px">
        <a-table style="width:100%;height:100%"
          ...
          <template slot="renderSeller" slot-scope="text">
...
<script>
  ...
  export default {
    ...
    created() {
      axios({
        method:'post',
        url:'http://java.xixibackup.me:8080/train/find',
        //url:'/train_all.json',
        data:{
          page:1,
          page_size:10,
      ...
      //this.data = data.data.info; //我这儿因为需要改了下，自己改回来
    },
    computed: {
    ...
    methods:{
      ...
      distribute(){
        ...
        this.createCheckForm.times=null;
        this.createCheckForm.amount=null;
        this.remark='';
        this.newPackageInfo='';
      },
      //新建训练分配
        ...
        this.$refs.createCheckForm.validate(valid=>{
          if(valid){
            ...
            let item={
              ...
              training_times:this.createCheckForm.times,
              training_amt:this.createCheckForm.amount,
            ...
            axios({
              method:'post',
              url:'http://java.xixibackup.me:8080/train/add',
              data:{
                busicd: "ORG_PRESENTATION",
                channel_id:111111,
                plan_id:22222,
                training_times: this.createCheckForm.times,
                training_amt: this.createCheckForm.amount,
      ...
      handleRepeal(){
        ...
        this.data[this.theKey-1].status="撤销";//检查这句话是否可以删除
        //前面的看看是不是可以都注释掉
        axios({
          method:'post',
          url:'http://java.xixibackup.me:8080/train/cancel',
          data:{
            orderno: this.data[this.theKey-1].orderno
          }
        }).then(res=>{
          console.log(res)
        }).catch(function (error) { // 请求失败处理
