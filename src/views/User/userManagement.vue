    ...
    <a-modal class="modal-430" :visible="model1visible" title="用户信息" @cancel="handChangeInfo" :maskStyle="{'background-color':'rgb(0,0,0,0.2)'}" width="400px" centered>
      ...
      <a-form-model :model="form2" :label-col="{span:6}" :wrapper-col="{span:18}" >
        ...
        <a-form-model-item label="重置密码">
          <div v-if="mima ===1">
            <span style="color:#F55943;cursor:pointer;text-decoration-line:underline;padding-right:15px;" @click="resetmima(2)">确定</span>
            <span style="color:#666666;cursor:pointer;text-decoration-line:underline;" @click="resetmima(2)">取消</span>
            <div style="color:#F55943;font-size:0.9em;line-height:20px;font-weight:bold;margin-top:-5px;">*注意：确认后密码将重置为系统初始密码</div>
          </div>
          <div v-if="mima ===2"><span style="color:#1079FA;cursor:pointer;" @click="resetmima(1)">重置</span></div>
        </a-form-model-item> 
      </a-form-model>
    </a-modal>
<!-- 修改处方弹窗 -->
